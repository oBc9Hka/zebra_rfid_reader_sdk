...
#if targetEnvironment(simulator)
    handleSimulator(call, result: result)
#else
    handleDevice(call, result: result)
#endif
  }

#if targetEnvironment(simulator)
  private func handleSimulator(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
    case "getAvailableReaderList":
      result("[]")
    case "disconnect", "setAntennaPower", "setDynamicPower", "setBeeperVolume",
         "connect", "findTheTag", "stopFindingTheTag":
      result(nil)
    default:
...
#else
  private func handleDevice(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
    case "getAvailableReaderList":
